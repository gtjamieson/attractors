...
//GJ_window.IS_IN_SIGHT = (window==window.top);
...
function Fish(population){
	...
	for(var i=0; i<100; i++){
		self.fishes.push({
			...
			speed: DIR * SPEED
//			speed: DIR * (SPEED + (Math.random() - 0.5) * 0.02)
...
self.update = function() {
	for (var i = 0; i < self.fishes.length; i++) {
		var f = self.fishes[i];
		f.x += f.speed;
		if (f.x >= self.width) f.x -= self.width;
	}
};
/*
	var buffer = 50;
...
*/
	self.draw = function(){
		...
		//  GJ inverse count 100-
		//		var count = 100-(Math.round(self.fishes.length * (pop.n/pop.max)));
		//  GJ count fish based on fear NOT population
		var count = (Math.round(self.fishes.length * (pop.thresholdUnder/pop.max)));
...
function PopulationSlider(population){
	...
	self.draw = function(ctx){
		...
		ctx.fillText("Doing", self.buttonX, self.top+7);
//		ctx.fillText("Willpower", self.left+(self.width/2), self.top+5);
...
function HillShaper(population, hill){
	...
	self.draw = function(ctx){
		...
		// Draw Under tab
		ctx.fillStyle = "hsl(55, 84%, "+(self.isHoveringUnderThreshold?70:59)+"%)";
		ctx.beginPath();
		ctx.arc(self.underButtonX, self.underButtonY, self.buttonRadius, Math.TAU/4, -Math.TAU/4);
		ctx.fill();

		// Draw Over tab
		ctx.fillStyle = "hsl(100, 84%, "+(self.isHoveringOverThreshold?70:59)+"%)";
		ctx.beginPath();
		...
		ctx.fillStyle = "#000";
		ctx.textAlign = "right";
		ctx.fillText("Fear", self.underButtonX, self.underButtonY+5);
		//GJ_Slider label
		ctx.font = '23px sans-serif';
		ctx.fillStyle = "#000";
		ctx.textAlign = "left";
		ctx.fillText("Desire", self.overButtonX, self.overButtonY+6);
...
//GJ_buttons to increase or decrease
	self.draw = function(ctx){
...
//addImage("water1", "https://storage.googleapis.com/enakr8ia_habitat/source_files/attractors-master/sim/img/water1.png");
addImage("water1", "https://storage.googleapis.com/enakr8ia_habitat/source_files/attractors-master/sim/img/laughingBuddha.png");
addImage("water2", "https://storage.googleapis.com/enakr8ia_habitat/source_files/attractors-master/sim/img/surprisedBuddha.png");
addImage("fish", "https://storage.googleapis.com/enakr8ia_habitat/source_files/attractors-master/sim/img/sim_wave.png");
addImage("ui_slide", "https://storage.googleapis.com/enakr8ia_habitat/source_files/attractors-master/sim/img/ui_slide.png");
addImage("ui_click", "https://storage.googleapis.com/enakr8ia_habitat/source_files/attractors-master/sim/img/ui_click.png");
...
SOUNDS.squeak_down = new Howl({
	src: ["https://storage.googleapis.com/enakr8ia_habitat/source_files/attractors-master/sim/sounds/squeak_down.mp3"],
	volume: 1.0
});
SOUNDS.squeak_up = new Howl({
	src: ["https://storage.googleapis.com/enakr8ia_habitat/source_files/attractors-master/sim/sounds/squeak_up.mp3"],
	volume: 1.0
});
SOUNDS.drag_down = new Howl({
	src: ["https://storage.googleapis.com/enakr8ia_habitat/source_files/attractors-master/sim/sounds/drag_down.mp3"],
	volume: 1.0
});
SOUNDS.drag_up = new Howl({
	src: ["https://storage.googleapis.com/enakr8ia_habitat/source_files/attractors-master/sim/sounds/drag_up.mp3"],
	volume: 1.0
});
SOUNDS.splash = new Howl({
	src: ["https://storage.googleapis.com/enakr8ia_habitat/source_files/attractors-master/sim/sounds/applause_sim.m4a"],
	volume: 1.0
});
SOUNDS.reel = new Howl({
	src: ["https://storage.googleapis.com/enakr8ia_habitat/source_files/attractors-master/sim/sounds/d'oh.mp3"],
	volume: 0.5
});
SOUNDS.gasp = new Howl({
	src: ["https://storage.googleapis.com/enakr8ia_habitat/source_files/attractors-master/sim/sounds/gasp.mp3"],
	volume: 1.0
});
